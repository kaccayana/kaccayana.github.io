...
I have sourced the text of Kaccāyana from **Chaṭṭha Saṅgāyana Tipiṭaka** available from the [Pāḷi Tipiṭaka web site]](https://tipitaka.org). This is based on the Chaṭṭha Saṅgāyana Tipiṭaka Version 4 (CST4) CD published by the [Vipassana Research Institute](https://www.vridhamma.org), available for download [on their website](https://tipitaka.org/cst4), and also available on [Github](https://github.com/fsnow/cst).

Unfortunately, the version of Kaccāyana available above contain many spelling errors. I have taken the opportunity to correct these errors whenever I have encountered them, but it is possible I have not have identified all the errors and some errors remain which may impact the translation.

The examples and counter-examples in the text are usually taken from the Pāḷi Tipiṭaka (and commentaries), In the translation, I have tried to identify the references where possible and cross referenced them against the [World Tipiṭaka Edition](https://tipitaka2500.github.io), but have not always succeeded. It is possible these examples are references to other text outside the canon.
...
1. Sandhi
   ...
   5. Miscellanea
2. Nāma
   1. Definitions, Internal morphology, Default Inflections
   2. Pronominal inflections, an-stems
   3. Pronominal inflections continued, mano-gaṇa, an-stems, tar-stems.
...
* `saññā` (technical term)
  * `anvattha` (meaningful)
    >**8, 10.Aṃiti niggahitaṃ.** <Badge text="anvattha" variant="note" /> \
    >Aṃ iti niggahitaṃ nāma hoti. \
    >Tena kvattho? Aṃ byañjane niggahitaṃ.
  * `rūḷhī` (artificial)
    >**58, 29.Ivaṇṇuvaṇṇā jhalā.** <Badge text="rūḷhī" variant="note" /> \
...
* `adhikāra` (domain)
  * `sīhagatika` (comprehensive) \
    >**52, 60.Jinavacanayuttaṃhi.** <Badge text="sīhagatika" variant="caution" />
    >"Jinavacanayuttaṃ hi" iccetaṃ adhikāratthaṃ veditabbaṃ.
  * `maṇḍūkagatika` (selective)
    >**131, 0.Itthipumanapuṃsakasaṅkhyaṃ.** <Badge text="maṇḍūkagatika" variant="caution" /> \
    >"Itthipumanapuṃsakasaṅkhyaṃ" iccetaṃ adhikāratthaṃ veditabbaṃ.
  * `yathānupubbika` (progressive)
    >**413, 427.Kāle.** <Badge text="yathānupubbika" variant="caution" />
    >"Kāle" iccetaṃ adhikāratthaṃ veditabbaṃ.
* `paribhāsā` (metarule)
  * `saññaṅga` (definition metarule)
    >**9, 11.Parasamaññā payoge.** <Badge text="saññaṅga" variant="tip" /> \
  ...
  * `vidhyaṅga` (operational metarule)
    >**10, 12.Pubbamadhoṭhita massaraṃ sarena viyojaye.** <Badge text="vidhyaṅga" variant="tip" /> \
...
An operation converts `from` into `to` surrounded by `before` and `after`, in the scope of `option` and `context`.
...
Context: "in the sense of ..."

Vibhatti endings are used to determine the various roles in the rule
...
> [!TIP] Example
> **§103 `ato n’ ena`**: (After a, nā [becomes] ena):
>
> | option | before | from | operation | to  | after | context |
> | ------ | ------ | ---- | --------- | --- | ----- | ---- |
> |        |    a   |  nā  |   ādesa   | ena |       |      |
...
> [!TIP] Example
> **§70: `jhalānam iyuvā sare vā`**: (Optionally, before a vowel: iya replaces i/ī, uva replaces u/ū):
>
> | option | before | from | operation | to  | after | context |
> | ------ | ------ | ---- | --------- | --- | ----- | ---- |
> |   vā   |        | i/ī  |   ādesa   | iva | sare  |      |
> |   vā   |        | u/ū  |   ādesa   | uva | sare  |      |
> |   vā   |        |   ⑥  | ādesa   |  ①  |   ⑦   |      |
>
> | | bhikkhu + āsane| rule |
> | --- | --- | --- |
> | → | bhikkh + (u→uva) + (āsane) | §70 |
> | → | bhikkh + uv + (~~a~~ + ā) + sane | §12 |
> | → | **bhikkhuvāsane** | |

> [!TIP] Example
> **§12 `sarā sare lopaṃ`**: (Vowel, before vowel, is elided):
>
> | option | before | from | operation | to  | after | context |
> | ------ | ------ | ---- | --------- | --- | ----- | ---- |
> |        |        | sarā | lopaṃ | ~~sara~~ | sare |      |
> |        |        |   ①  | lopaṃ   |     |   ⑦   |      |
>
> | | tena + upasaṅkami | rule |
> | --- | --- | --- |
> | → | ten(~~a~~) + upasaṅkami | §12 |
> | → | **tenupasaṅkami** | |
