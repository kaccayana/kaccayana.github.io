...
> [!NOTE]
> `akkhara` means “constant, durable, long-lasting” and in the context of Kaccāyana it means “sounds, tones, words” and also the letters of the `abugida` (alpha-syllabary). It is derived from Akshara (Sanskrit: अक्षर, romanized: akṣara, lit. 'imperishable, indestructible, fixed, immutable'), a term used in the traditional grammar of the Sanskrit language and in the Vedanta school of Indian philosophy.
...
All meanings of words are based on the recognition of “akkhara” (sounds, or units of the writing system). A mistaken akkhara result in difficulty in interpreting meaning, therefore proficiency in akkhara is extremely helpful in (deciphering) the rules (or understanding the suttas in the Tipiṭaka).
...
### §2 (2): Akkharāpādayo ekacattālīsaṃ (41 akkhara starting with `a`) :badge[anvattha]{variant=success}
...
### §3 (3): Tatthodantā sarā aṭṭha ([From `akkhara`] eight ending with `o`: `sara`)  :badge[anvattha]{variant=success}
...
> [!TIP] Reference
> Refer to [¶12: sarā sare lopaṃ (elision of vowel from vowels)](#m12)

---

<a name="m4"></a>

### §4 (4): Lahumattā tayo rassā (3 short vowels: `rassa`) :badge[anvattha]{variant=success}
...
### §5 (5): Aññe dīghā (Other [`sarā`] are `dīgha`) :badge[anvattha]{variant=success}
...
### §6 (8): Sesā byañjanā (Remaining are `byañjanā`) :badge[anvattha]{variant=success}
...
### §7 (9): Vaggā pañcapañcaso mantā (`vagga`: 5 by 5 [byañjane] ending with `ma`)  :badge[anvattha]{variant=success}
...
### §8 (10): Aṃ iti niggahitaṃ (`aṃ` is denoted `niggahita`) :badge[anvattha]{variant=success}
...
### §9 (11): Parasamaññā payoge (Other grammatical terms used) :badge[sannanga]{variant=tip}
...
### §10 (12): Pubbamadhoṭhita massaraṃ sarena viyojaye (Detach vowel from preceeding/succeeding [unit, to create] vowelless [consonants]) :badge[vidhyanga]{variant=tip}

> [!NOTE]
> Note that this is a meta-operational rule that makes sense when referring to abugida scripts like Brahmi where the vowels are attached to consonants in words. In Romanised Pāli, the vowels and consonants are represented by distinct letters so separating the vowel from the consonant is inherently done.
...
### §11 (14): Naye paraṃ yutte (Connect next if applicable) :badge[vidhyanga]{variant=tip}
...
### §12 (13): Sarā sare lopaṃ (Before vowel, vowel is elided) :badge[utsarga]{variant=note}

> [!NOTE] Analysis
> | option | before | from | operation | to | after |
> | --- | --- | --- | --- | --- | --- |
> | | sarā | sare | lopaṃ | | |
...
### §13 (15): Vā paro asarūpā (Optionally, next dissimilar [vowel]) :badge[apavada]{variant=note}
